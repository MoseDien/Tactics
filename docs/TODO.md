...
- **「立即下载下一块」**(2026-09-03 新增):`minimum: .max` 强制跳过池充足判断,直接走真实下载路径,弹窗按 `ProvisionOutcome` 显示导入 N 题 / 未发布 404 / 失败 / 跳过——无需等冷却、无需重启,10 秒闭环。
- **「回到初始状态」**(2026-09-03 新增,destructive,带确认):`deleteAllData()`(四表全清,含已下载块)+ `AppPreferences.wipeAll()`(rating/batch/难度/序号/gate 六个 key 全清)→ gate 清空后 RootView 自动路由回导入页重导内置 0000 块——等价删除重装,无需重启。
- 既有工具:「清空未尝试题池」(批量标记已尝试,测自动触发路径:排干 → 冷却结束开新组 → 自动拉块)、「重置数据块序号」。
- 前置:`RemotePuzzleCatalog.baseURL` 指向 https://mosedien.github.io/Tactics/puzzles/ ,块 0001–0010 已发布。
- 典型闭环:回到初始状态 → 确认「数据块 0 · 1000 题」→ 立即下载下一块 → 「数据块 1 · 2000 题」;断网点立即下载 → 失败弹窗且状态不变;再点重置 → 回 0/1000。
