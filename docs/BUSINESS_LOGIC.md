...
- 冷却期间重新打开 App 不会随机生成新题，只进入当前 round 的 Review mode;**窗口过期后重开同样进入 Review**(开新轮只能由用户点 `Next round` 触发，启动永不自动开新轮)。
...
题目完成后，Hint 按钮不再影响解题或 Rating；点击会弹出该题的单题复盘，可逐步查看完整解法。
